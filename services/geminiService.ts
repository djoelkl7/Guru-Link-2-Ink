...
function extractSourcesText(response: any): string {
    const chunks = response.candidates?.[0]?.groundingMetadata?.groundingChunks;
    if (!chunks) return "";
    
    const uniqueUris = new Set<string>();
    chunks.forEach((chunk: any) => {
        if (chunk.web?.uri) uniqueUris.add(chunk.web.uri);
    });
    
    if (uniqueUris.size === 0) return "";
    
    return "\n\nSources:\n" + Array.from(uniqueUris).map(uri => `- ${uri}`).join('\n');
}
...
export async function askRepoQuestion(question: string, infographicBase64: string, fileTree: RepoFileTree[]): Promise<string> {
  ...
  try {
    ...
    const text = response.text || "I couldn't generate an answer at this time.";
    return text + extractSourcesText(response);
  } catch (error) {
...
export async function generateArticleInfographic(
  ...
  aspectRatio: string = "1:1"
    ...
    try {
        ...
        // Switch to 'gemini-2.5-flash' for research phase as requested.
    ...
    if (onProgress) onProgress(count > 1 ? `DESIGNING ${count} VARIATIONS...` : "DESIGNING & RENDERING INFOGRAPHIC...");
    ...
    const generateImage = async (index: number) => {
        // Slight variation in prompt to ensure diversity if multiple are requested
        const variationInstruction = count > 1 ? `VARIATION ${index + 1}: focus on a unique layout composition.` : "";
    ...
    try {
        // Execute requests in parallel (or sequential if limits are tight, but parallel is better for UX)
        const imagePromises = Array.from({ length: count }, (_, i) => generateImage(i));
        const results = await Promise.all(imagePromises);
        const validImages = results.filter((img): img is string => img !== null);
        
        return { images: validImages, citations };
    } catch (error) {
