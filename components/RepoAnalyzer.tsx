...
import { AlertCircle, Loader2, Layers, Box, Download, Sparkles, Command, Palette, Globe, Clock, Maximize, KeyRound, Folder, FileCode } from 'lucide-react';
...
// --- Sub-component for Visualizing File Structure ---
const StructureVisualizer: React.FC<{ fileTree: RepoFileTree[] }> = ({ fileTree }) => {
  const stats = useMemo(() => {
     ...
     fileTree.forEach(f => {
         ...
         folders[root] = (folders[root] || 0) + 1;
         
         const ext = f.path.split('.').pop()?.toLowerCase() || 'misc';
         languages[ext] = (languages[ext] || 0) + 1;
     });
     
     const sortedFolders = Object.entries(folders)
        .sort(([,a], [,b]) => b - a)
        .map(([name, count]) => ({ name, count, percent: (count / fileTree.length) * 100 }));
        
     const sortedLangs = Object.entries(languages)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 6); // Top 6

     return { folders: sortedFolders, languages: sortedLangs };
  }, [fileTree]);

  return (
    <div className="glass-panel rounded-3xl p-6 mb-6 animate-in fade-in slide-in-from-bottom-4">
        {/* Header */}
        <div className="flex items-center gap-2 mb-6 border-b border-white/5 pb-4">
            <div className="p-2 bg-blue-500/20 rounded-lg text-blue-300 border border-blue-500/30">
                <Box className="w-5 h-5" />
            </div>
            <div>
                <h3 className="text-lg font-bold text-white font-sans tracking-tight">Repository Structure</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Treemap / Folder Distribution */}
            <div className="lg:col-span-2 space-y-4">
                 <h4 className="text-xs font-mono text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <Folder className="w-3 h-3" /> Directory Topology
                 </h4>
                 <div className="flex flex-wrap gap-2 w-full">
                    {stats.folders.map((folder, i) => (
                        <div 
                            key={folder.name}
                            className="bg-slate-800/40 hover:bg-slate-700/60 border border-white/5 hover:border-violet-500/30 rounded-lg p-3 flex flex-col justify-between transition-all relative overflow-hidden group cursor-default"
                            style={{ 
                                flexBasis: `${Math.max(15, folder.percent * 1.5)}%`,
                                flexGrow: 1,
                                minWidth: '100px' 
                            }}
                        >
                             {/* Background graph bar effect */}
                             <div className="absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-violet-500 to-blue-500 opacity-50 group-hover:opacity-100 transition-opacity" style={{ width: `${folder.percent}%` }}></div>
                             
                             <div className="flex justify-between items-start z-10 gap-2">
                                 <span className="font-mono text-sm font-bold text-slate-300 truncate" title={folder.name}>{folder.name}</span>
                                 <span className="text-[10px] text-slate-400 font-mono bg-white/5 px-1.5 py-0.5 rounded border border-white/5">{folder.count}</span>
                             </div>
                             <div className="mt-2 text-[10px] text-slate-600 font-mono group-hover:text-slate-400 transition-colors">
                                 {folder.percent.toFixed(1)}% VOL
                             </div>
                        </div>
                    ))}
                 </div>
            ...
            <div className="space-y-4">
                 <h4 className="text-xs font-mono text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                    <FileCode className="w-3 h-3" /> Language Composition
                 </h4>
                 <div className="space-y-3 bg-slate-900/30 p-4 rounded-xl border border-white/5">
                    {stats.languages.map(([lang, count], i) => {
                        const percent = (count / fileTree.length) * 100;
                        const colorClass = i === 0 ? 'bg-violet-500' : i === 1 ? 'bg-blue-500' : i === 2 ? 'bg-emerald-500' : 'bg-slate-500';
                        return (
                            <div key={lang} className="group">
                                <div className="flex justify-between text-xs mb-1.5">
                                    <span className="font-mono text-slate-300 uppercase font-semibold">{lang}</span>
                                    <span className="text-slate-500 font-mono">{percent.toFixed(1)}%</span>
                                </div>
                                <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
                                    <div 
                                        className={`h-full rounded-full ${colorClass} shadow-[0_0_10px_currentColor] opacity-80 group-hover:opacity-100 transition-opacity`} 
                                        style={{ width: `${percent}%` }}
                                    ></div>
                                </div>
                            </div>
    ...
    <div className="max-w-6xl mx-auto space-y-10 mb-20">
      ...
      {infographicData && !loading && (
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
          ...
          {currentFileTree && <StructureVisualizer fileTree={currentFileTree} />}
